...
// 这个是我随便找的，2251998486这个数字应该不对，但是这个宏肯定在吧
...
- AutoInitializeUMyTest初始化，最终添加到DeferredClassRegistration这个数组里面的是FFieldCompiledInInfo这个结构，主要是里面有个 Register方法，通过 Register方法能够调用GetPrivateStaticClass方法，进而创建出此类的UClass（这个过程后面还会讲一下）。
