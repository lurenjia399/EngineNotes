...
c++构造函数中创建的Subobject都是路径稳定，因为newObject创建c++对象的时候这些就会创建吧，就会加载到内存中了，通过findObject就能找到，
4 newObject的时候，object身上挂着的component也都会进入内存吧，所以C++构造函数中创建的SubObject也都是路径稳定的？
5 客户端GuidCache->IsExportingNetGUIDBunch这个标志位在哪读出来的？
6 加载了Package，package里面的Object都会加载么？
