...
> ActorA同步UObjectB属性应该是通过属性同步的流程，但属性同步的流程只是同步了UObject,也就是让客户端创建出了UObjectB，没有同步UObjectB的属性，所以还需要重写actor的ReplicateSubObjcet方法来绒布UObjectB的属性，这时客户端就会创建出UObjectC了，但是UObjectC的属性也没有同步下来，所以还需要重写UObject的ReplicateSubObjcet方法来同步UObjectC的属性。这里需要重写ActorA的ReplicateSubObjcet方法的原因是默认的ReplicateSubObjcet这个只会同步component，而UObject需要我们自己加上。
