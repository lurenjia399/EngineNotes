...
> Actor同步UObject类型的属性应该是通过属性同步的流程，但属性同步的流程只是同步了Actor的属性，没有同步属性的属性，所以还需要重写ReplicateSubObjcet方法来同步属性的属性。Actor同步Uobject类型应该是需要重写ReplicateSubobject方法，将UObject纳入到SubObject同步流程中。属性中还有个可同步的UObject，也需要重写ReplicateSubobject方法。所以要想同步的就需要重写Actor::ReplicateSubObject方法和UObject::
