...
> 看上去属性同步的时候，会将属性写入到Bunch里面，如果是UObject类型的就会调用serializeObject来写入，这个方法不会写入属性。重写的ReplicateSubobject方法首先会找到SubObject的ObjectReplicator，进而调用ReplicateProperties方法来将SubObject的属性写入到Bunch。
