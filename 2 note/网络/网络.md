...
> 情况2：在同一帧中A比B先同步过来，这就会导致在赋值A成员变量的时候找不到B，然后就会把B的Replicator放到UnMappedReplicators数组里面，随后TickFlush方法里面处理当前帧解析出来Object
> 但是B作为成员变量，他的NetGUID也会写入A中属性同步Bunch里面发送到客户端，如果客户端收到的时候B已经同步过了，那么B就会在NetGUIDCache里面找到然后赋值上，如果客户端收到的时候B还没同步下来，就会先将Bunch放到UnMappedReplicators里面，随后在TickFlush方法里面处理UnMappedReplicators数组。
