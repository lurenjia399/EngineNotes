...
3 AActor->RemoteRole != ROLE_None 建立Actor是否参数属性同步，也就是被Replicated标记。对于可同步的Actor，服务器上RemoteRole是ROLE_SimulatedProxy，Role是ROLE_Authority，客户端上RemoteRole是ROLE_Authority，本地Role是ROLE_AutonomousProxy，模拟Role是ROLE_SimulatedProxy。对于客户端创建的Actor，Role是ROLE_Authority，被Replicated标记的话RemoteRole是
*/
```
### 2 UFUNCTION(Server)
```cpp
...
执行空间：
1 在服务器调用时仅本地执行
2 在客户端调用时，对于本地权威角色（客户端生成的仅客户端存在Actor），若其参与了属性同步（Actor被Replicated标记）且有所属链接（Actor的Owner是不是PlayerController）远端执行，否则本地执行
*/
```
# 问题：
