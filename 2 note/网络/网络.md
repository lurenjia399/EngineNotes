...
1 第一部分，这部分的开始是在ActorChannel的创建完成后会执行SetChannelActor方法将Actor和ActorChannel相关联，在这个方法里面会执行ObjectReplicator的创建，进而触发RepLayout，RepState，TrackerRepState的创建。RepLayout里会将可复制的属性信息放到Parents和cmds数组中以及复制条件啥的，总的来说作用是存储可复制属性的布局。
