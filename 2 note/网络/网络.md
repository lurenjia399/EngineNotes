...
> 一样的流程，如果属性是动态的，那么客户端就会通过NewObject创建一个出来然后赋值上。如果属性是静态的，那么客户端就会在内存中找Object，如果找到了就赋值，如果找不到就加载一个，如果是异步加载就无法立即赋值，就会把Bunch放到QueueBunch里面，等到TickFlush在执行。注意Actor
