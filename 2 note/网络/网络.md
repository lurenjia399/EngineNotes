...
```
在UNetDriver::InitBase里面(也就是基类里面)，创建了NetConnectionClass，是通过BaseEngine.ini里面的NetConnectionClassName配置创建出IpConnection类的。然后就是设置了FNetworkNotify类型的成员变量Notify，在服务器这部分这个Notify是当前的World，也就是调用Listen的World。最后如果NetConnectionClass创建成功就返回true。
