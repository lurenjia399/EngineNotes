...
> 首先ExportBunches里面存放的这个ActorChannel中Actor的那些静态的UObject信息，比如Actor的CDO，CDO的Outer（UPackage），Actor身上挂着的Component，以及这些Component的CDO，CDO的Outer（UPackage）。在客户端的ReceivedRawBunch这个方法中，就会优先解析ExportBunch里的内容，就是加载这些静态Object，然后注册NetGUID。然后通过ReadContentBlockPayload读取Component信息
