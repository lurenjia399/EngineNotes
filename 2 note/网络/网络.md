...
ERepLayoutResult FRepLayout::CompareProperties(...) const
{
	...
	// 拿到我们这次要处理的RepChangedHistory，就是ChangeHistory数组里面的一个元素，这个元素里有Changed数组，这个数组存储的应该是Object中变化属性的的索引
	...
	// 如果保存的改变次数超过了ChangeHistory数组的大小，就需要合并其中的元素，每次合并都是合并0号元素和1号元素，将合并的内容放到1号元素中，这样0号元素就空了，我们新加的这个Changed就放在0号元素上，这样类似一个循环数组？这样循环下去
