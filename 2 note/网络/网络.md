...
第一部分，通过CreateReplicatorForNewActorChannel创建出NewReplicator，并在这个方法里做一些初始化，后面详细看下这个方法。
第二部分，将obj和NewReplicator添加到ReplicationMap里面缓存。
第三部分，StartReplicating方法，再做一些初始化的东西，后面详细看下这个方法。
```cpp
