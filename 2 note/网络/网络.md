...
3 在客户端调用时，对于非本地权威角色，若其参与了属性同步且有所属链接远端执行，否则终止执行
*/
...
```
注意：
1 在客户端的本地权威角色的Actor，其是在客户端创建，所以服务器没有这个Actor，所以无论是将其的bReplicated设置成true又或者是将其owner设置成PlayerController，他都无法在服务器执行调用。所以上面情况2说明的是远端执行，
2 
有几个小问题：
