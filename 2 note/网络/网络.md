...
/*
	...
	第二部分：一个循环遍历，遍历Bunch里面的内容，看上去就是遍历actor身上的subobject对象（也就是owner为 当前actorChannel里actor 的subobject对象），RepObj变量就是subobject，然后也会创建其Replicator，进而保存到actorChannel里的ReplicationMap这个map里面。
