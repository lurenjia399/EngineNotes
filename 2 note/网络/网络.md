...
2 第二部分，这部分的开始是在ActorReplicator::ReplicateProperties这个方法中，这个方法中首先会比较属性是否改变，将改变属性的序号记录到RepChangelistMgr的RepChangelistState结构中，然后回将改变的属性序列化到局部Bunch里面，然后会序列化自定义条件属性，再然后会序列化RPC，
