...
bool FObjectReplicator::ReplicateProperties(...)
{
	...
	// 参数1 ： 我们之前setActorChannel里面创建的RepLayout
	// 参数2 ： 我们之前setActorChannel里面创建的RepState
	// 参数3 ： 在服务器上，StartReplicating方法里创建的，里面存储了StaticBuffer
	// 参数4 ： 需要比较属性的对象UObject
	// 参数5 ： 记录的帧数，上一次属性改变，需要同步的帧数
	// 参数6 ： FReplicationFlags结构体，是个union
