...
- RPC的UFunction里面可以放裸c++结构体
1 UFunction会自动把函数参数都创建FProperty
2 裸c++写NetSerialize方法，就可以同步
- 服务器上Actor广播NetMusticast方法，所有的客户端和服务器都能执行，但另外的ClientRPC和ServerRPC必须又所属链接才能执行。
- 一个可同步的ActorA，这个ActorA有一个需要同步的ActorB，这个ActorB的Replicates是false，所以ActorA的这个ActorB属性是不会同步的,
