...
# 3 Actor属性同步
## 1 Server
还记得我们在UNetDriver::ServerReplicateActors_ProcessPrioritizedActors这个方法里面，如果actor的actorChnnel不存在，我们就会创建一个新的actorChannel，在创建完成后我们会调用SetChannelActor方法，将ActorChannel和Actor关联起来。
```cpp
void UActorChannel::SetChannelActor(AActor* InActor, ESetChannelActorFlags Flags)
{
	...
	if (Actor)
	{
		// 将actor和actorchannel的关联保存到connection的
