...
// 注意情况3，客户但非本地权威角色的Actor，这种Actor肯定参与了属性同步但只有autonomous的有所属链接（AActor::GetNetConnection返回不为nullptr），而simulated的没有所属链接（AActor::GetNetConnection返回nullptr），所以autonomous的可以调用RPC而simulated不能调用，但是将simulated的Actor的Owner设置成PlayerController，simulated也就可以调用RPC了。
...
>服务器上会存在ClientNetConnections数组，数组里每个元素都会有个PlayerController，所以每个客户端上的PlayerController都是不一样的。
...
> Actor同步UObject类型的属性应该是通过属性同步的流程，但属性同步的流程只是同步了Actor的属性，没有同步属性的属性，所以还需要重写ReplicateSubObjcet方法来同步属性的属性。
