...
- 同步Actor是流程是什么呢？
继续上边描述的，起始位置是ReplicateActor方法，然后会调用SerializeNewActor方法来序列化Actor，再SerializeNewActor方法里调用SerializeObject来序列化Actor（将Actor的NetGUID注册，然后将NetGUID写入Bunch里），然后会再次调用SerializeObject来学历恶化Actor的Archetype（一般Archetype都是uclass中的cdo，所以是网络稳定。在通过SerializeObject序列化网络稳定Object的时候会将Object的NetGUID，Object的Flags以及outer的NetGUID，outer的Flags以及outer的路径和Object的路径写入ExportBunch里面，在最后SendBunch的时候会将ExportBunch写在Bunch的前部）。然后会再次调用SerializeObject来xu
