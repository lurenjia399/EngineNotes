...
> Actor都是通过创建ActorChannel来进行同步的。但是B作为成员变量，他的NetGUID也会写入A中属性同步Bunch里面发送到客户端，如果客户端收到的时候B已经同步过了，那么B就会在NetGUIDCache里面找到然后赋值上，如果客户端收到的时候B还没同步下来，就会先将Bunch放到UnMappedBunch里面，
