...
### 1 Server FObjectReplicator中RepLayout 创建
```cpp
...
```
这个主要就是InitWithObject这个方法，初始化Replicator一些成员变量，其中就有创建RepLayout，然后就是初始化同步数据的这个InitRecentProperties方法，最后就是把自己缓存起来。
#### 1 UNetDriver::GetObjectClassRepLayout
