...
```
这个方法还是会执行CompareParentProperties方法，继续往里执行比较属性。
这个方法其他的部分就是对FRepChangelistState这个结构体的操作，这个机构里有ChangeHistory成员变量，其的大小是64，里面每个元素是个数组存储的是发生变化的属性序号。最后还会有ChangeHistory合并数组元素的操作，因为大小是64固定，要想继续存只能合并了。
