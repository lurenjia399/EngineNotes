...
bool UPackageMapClient::SerializeObject( FArchive& Ar, UClass* Class, UObject*& Object, FNetworkGUID *OutNetGUID)
{
	...
	if (Ar.IsSaving())
	{
		...
		if (!NetGUID.IsDefault() && ShouldSendFullPath(Object, NetGUID))
		{
			...
			// 多次调用InternalWriteObject方法，每次都会写入NetGUID，但写入的不是Ar，是CurrentExportBunch这个Bunch，这个Bunch他是装满了然后会放到这个ExportBunches数组里面，然后在SengBunch里，遍历ExportBunches数组
...
#### 5 2 UPackageMapClient::InternalWriteObject UPackageMapClient::InternalLoadObject
```cpp
