...
- 同步Actor是流程是什么呢？
继续上边描述的，起始位置是ReplicateActor方法，然后会调用SerializeNewActor方法来序列化Actor，再SerializeNewActor方法里调用SerializeObject来序列化Actor（将Actor的NetGUID注册，然后将NetGUID写入Bunch里），然后会再次调用SerializeObject来学历恶化Actor的Archetype（），
