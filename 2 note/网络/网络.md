...
- 同步Actor的属性流程是什么呢？
同步SubObject是在同步Actor的基础之上，所以也是从TickFlush开始，经历一系列的方法到ReplicateActor方法中，在SerializeNewAcdtor方法之后调用的ActorReplicator::ReplicateProperties方法里同步。整个属性同步流程就分成两个部分，第一个部分是初始化创建，第二部分是比较属性改变进而写入Bunch。
1 第一部分，这部分的开始是在ActorChannel的创建完成后会执行SetChannelActor方法将Actor和ActorChannel相关联，在这个方法里面会执行ObjectReplicator的创建，进而触发RepLayout，RepState，RepChangelistMgr，RepChangedTracker的创建。RepLayout里会将可复制的属性信息放到Parents和cmds数组中以及复制条件啥的，总的来说作用是存储可复制属性的布局。RepState会用StaticBuffer记录属性的副本，客户端属性改变的比较判断就是通过StaticBuffer来完成的。而服务器的StaticBuffer是存储在RepChangelistMgr里面的，来记录属性的副本和RepState中的StaticBuffer区别可能就是listMgr里面会记录属性过去的历史数据，而RepState里面只会记录属性的当前历史数据。
