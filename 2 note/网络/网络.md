...
3 AActor->RemoteRole != ROLE_None 建立Actor是否参数属性同步，也就是被Replicated标记。对于可同步的Actor，服务器上RemoteRole是ROLE_SimulatedProxy，Role是客户端上RemoteRole是ROLE_Authority，客户端上RemoteRole是ROLE_Authority，
*/
```
### 2 UFUNCTION(Server)
```cpp
