			...
			for (int32 i = 0; i < NumMustBeMappedGUIDs; i++)
			{
				...
				if (FNetGuidCacheObject const * const GuidCacheObject = GuidCache->GetCacheObject(NetGUID))
				{
					...
					// Cache中的Object没有加载出来
					{
						...
						// 在添加QueuedBunchObjectReferences数组，静态的Object客户端无法求解出Object，就会先放入到QueuedBunchObjectReferences数组里面，然后等到Object加载完成后会通过UpdateQueuedBunchObjectReference方法来更新
...
- 客户端什么情况下无法通过NetGUID立即求解出Object的情况？有两种处理方式？都是怎么处理的？
