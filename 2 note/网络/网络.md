...
2 每一帧会把所有的Actor都同步么？
不会。每一帧会都会构建需要同步的Actor数组并计算下一次需要同步的时间（这个时间是当前world时间 + 小范围的随机值 + Actor的同步频率的配置），每次在构建同步数组的时间会看是否到达了时间，如果到了才有可能加入数组中。每一个ClientConnection都会有可用的空间长度，每次同步了一个Actor之后会计算这个空间长度是否