...
> 用于客户端标记收到的Bunch中是否有MustBeMapped部分，这个部分保存的是开启异步加载情况下路径稳定的NetGUID，这个变量会在ReceivedRawBunch里面赋值，并尝试优先解析MustBeMappedGUID。
...
- UClass是网络稳定的么？
> 看样子是稳定的，UClass的生成是在执行main函数之前的static变量中调用的，也就是服务器客户端都存在，自然算是网络稳定的。
...
>当使用 Archetype 来创建对象实例时，该对象会从特定 Archetype 复制默认属性，而不是从类默认对象（CDO）复制。这种方式允许开发人员创建具有特定配置的对象实例，而不需要改变全局类默认值。
>
>
