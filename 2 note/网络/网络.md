...
>当使用 Archetype 来创建对象实例时，该对象会从特定 Archetype 复制默认属性，而不是从类默认对象（CDO）复制。这种方式允许开发人员创建具有特定配置的对象实例，而不需要改变全局类默认值。2. 对象是在运行时动态生成的
运行时动态生成的对象可以通过特定的模板（Archetype）来创建，这样它们不会直接使用 CDO 的默认值。
示例：
在游戏运行时，你可能会从某个特定的 Archetype 动态生成一个角色或物品，而不是使用默认的类实例化方法。这种情况下，生成的对象的默认属性是基于 Archetype 的值，而非 CDO 的值。
>
>
