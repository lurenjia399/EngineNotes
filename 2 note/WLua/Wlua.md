...
> 然后就是lua通过userdata来表示UObject对象，c++创建userdata并将UObject和userdata映射保存到管理类中，在c++GC的时候通过UObject在resgister表中找到然后置空，然后会通过UObject的名称从registry表里找对应元表，找到后就设置userdata的元表，并将UObject对应的userdata保存到registry表里key为lightuserdata。