...
- lua是怎么和c++交互的呢？
> 总共分成了几部分，1 lua如何表示UObject对象。2 lua如何模拟UObject对象的继承关系。3 lua如何调用到UObject的方法。4 c++如何调用lua的方法。第一部分是c++通过UObject来创建userdata，并将其保存到全局弱表中，随后就根据UObject的UClass中的方法来设置userdata的元表，并将元表名称设成UClassName，并保存到全局表中。