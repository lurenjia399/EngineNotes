...
> 第一部分是c++通过UObject来创建userdata，然后通过UObject的UClass来创建元表，并将元表保存到_G全局表和register表中key都为UClassName，然后设置userdata的元表以及将userdata存到register表里，key为UObject的lightuserdata。第二部分