...
```
最终看下来就是localplayer的playercontroller会在创建localplayer的地方就创建，但是这是在客户端自己创建的，也就是role是authority，remoterole是none。在loadmap之后客户端给服务器发消息，然后服务器创建出可复制的PlayerController，
# 3 总结
