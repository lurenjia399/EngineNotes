...
```
字面意思不是很复杂，在这个方法里面，首先需要创建出设置好的GameInstance进而创建一系列的WorldContext，也就是说GameInstance是客户端服务器都存在的。然后如果是客户端或者是单机的话，就创建出ViewportClient，进而创建出LocalPlayer。
...
ULocalPlayer* UGameInstance::CreateLocalPlayer(...)
{
	...
	if (LocalPlayers.Num() < MaxSplitscreenPlayers)
	{
		...
		{
			// 如果不是客户端，单机就会走进来吧，就创建PlayerController
...
void UNetConnection::HandleClientPlayer(...)
{
	...
	// 紧接着就重新赋值playerController，并设置一系列的参数
...
```
最终看下来就是localplayer的playercontroller会在创建localplayer的地方就创建，但是这是在客户端自己创建的，也就是role是authority，remoterole是none。在loadmap之后客户端给服务器发消息，然后服务器创建出可复制的PlayerController，然后通过同步替换掉客户端创建的这个playercontroller。
# 3 总结
总的来说，LocalPlayer是在GameEngine::Init方法里，在创建完GameInstance之后，在创建这个UGameViewportClient里面创建的，并且只在客户端创建出来，这个创建的部分没有创建出对应的PlayerController。然后在LoadMap方法中，客户端和单机会通过GameInstance先将旧的LocalPalyer对应的PlayerController销毁掉，然后在新的World中创建出PlayerController。最后还需注意的是，通过LocalPlayer::SpawnPlayActor这个方法创建的PlayerController是一个Dummy的PlayerController，之后会从服务器复制一个把这个假的替换掉（Look at APlayerController::OnActorChannelOpen + UNetConnection::HandleClientPlayer for the code the  replaces this fake player controller with the real replicated one from the server）。
