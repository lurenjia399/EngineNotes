...
3 MarkReferencedClustersAsReachable：把簇中引用的object都标记为可达。第一步处理簇引用的其他簇，如果其他簇不是垃圾，就标记为可达，如果是垃圾就清掉引用。第二步处理簇中MutableObject（不在簇中但依然被簇引用。看上去是object属于多个簇，但只在一个簇中这种情况。），如果Object不是垃圾就将其添加到InitialObjects数组中
