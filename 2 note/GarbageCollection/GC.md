...
- 如何追踪指针引用关系改变的
- ARO是什么，哪里会使用呢？
ARO函数有两种，第一种是FGCObject的纯虚函数AddReferencedObjects，继承自GCObject的就必须要重写。第二种是继承UObject类型的静态AddReferencedObjects方法。第一种ARO是在可达性分析的开始会遍历继承自GCObject，然后执行其ARO方法，将引用的Object都纳入可达性分析的流程里。第二种ARO是再创建Schema的时候，如果重写了静态ARO方法，就会将ARO函数指针填充到Schema的Member中，再查看UObject的引用遍历schema的Member时候，会首先遍历到ARO方法，然后查看引用的对象有哪些。
- gc多线程是怎么使用的
