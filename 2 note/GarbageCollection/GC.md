...
```cpp
// 构建FSchemaView。参数传进来的ARO如果不是是UObject的那个虚函数AddReferencedObjects，才会有值，否则参数就为nullptr。看上去蓝图的UClass是自己设置的ARO，是AddReferencedObjectsInUbergraphFrame这个
...
看上去是再VisitMembers的ARO这个case下，走到HandleObjectReference才会处理PendingKill
- 如何让一个UObject不被gc回收
- 如何加速gc流程
