...
## 2.1 标记Object，HandleKillableReference
```cpp
...
第一帧ObjectA是可能不可达的，说明第一帧没有遍历到ObjectA，第二帧变成可达了，说明第二帧遍历到了，这非常正确，因为遍历到了所以标为可达。还有一种情况是第二帧变成可达，是因为AddToRoot方法，添加到根Object数组中了，在第三帧的可达性分析的开始就将会将ObjectA添加到遍历数组中，然后将其标记成可达的。
- 如果是增量标记，第一帧将ObjectA标记成可达的，第二帧被用户标记成pendingkill了，标记会改变么？
第一帧是可达的，那说明第一帧遍历到了ObjectA，因为第一帧遍历到了所以以后的所以帧都不遍历到了。所以就算是第二帧用户标记成pendingkill了，他最终也是可达的。
- 如何追踪指针引用关系改变的
每次可达性分析都是从根节点出来，深度优先遍历其引用，遍历到的标记成可达，未遍历到的就是可能不可达。
- ARO是什么，哪里会使用呢？
...
- gc多线程是怎么使用的
1 加速遍历数组，我们在将所有的根Object标记成可达的时候，就是拿到根Object数组用多线程遍历来加速。
2 加速可达性分析，是将根Object数组分给不同的线程去可达性分析，和多线程遍历数组一样。
- FUObjectHashTables这个hash桶是干嘛的
看上去就是把Object的名称hash了一下，为了寻找Object的时候更快速吧，是在UObjectBase的构造函数中hash的。
- LinkerLoad是每个UObject都有啊
LinkerLoad是在UPackage中，UPackage试继承自UObject的。应该是那种资源才会有LinkerLoad，我们用代码NewObject创建出的Object不会有LinkerLoad。LinkerLoad的作用就是将蓝图资源加载内存当中，并初始化ImportMap和ExportMap数组。
...
// Actor的PendingKill判断，就是判断是不是销毁了或者是带有垃圾RF_MirroredGarbage标志
...
- TSharedPtr修饰非Object类型，TSharedPtr指针修饰的c++类型Object，不会被gc么？
