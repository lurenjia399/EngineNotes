...
```
## 2 UObjectProcessRegistrants 进一步注册UClass
```cpp
...
*/
```
## 3 ProcessNewlyLoadedUObjects 注册UClass反射信息，生成CDO
```cpp
...
3. 注册UClass就分成了创建UClass通过GetPrivateStaticClassBody这个方法，进一步填充UClass中信息通过UObjectForceRegistration这个方法，然后就是填充UClass中的反射信息通过UObjectLoadAllCompiledInDefaultProperties这个方法，这个方法也会创建CDO，并且创建CDO是在构造函数执行之前。

通过uht分析.h文件中的宏来生成对应的.generated.h和.gen.cpp文件，里面会生成静态的方法和静态变量，静态变量的作用就是将类的信息收集到全局的静态数组中。等到模块加载的时候，遍历这个静态数组，调用到类的静态方法里面生成对应的uclass，然后还会把反射信息啥的都塞到uclass里面，进一步创建cdo以及调用构造函数。

切入函数（ProcessNewlyLoadedUObjects），这个是每个模块加载完城后会调用到这里：
