...
*/
// 这个方法最终会调用到GetPrivateStaticClass这个里面，也就是创建出UClass了
...
const UE4CodeGen_Private::FClassParams Z_Construct_UClass_UMyTest_Statics::ClassParams = {
	&UMyTest::StaticClass,// 名称（ClassNoRegisterFunc）
	nullptr,// 名称（ClassConfigNameUTF8）
	&StaticCppClassTypeInfo,// 名称（CppClassInfo）
	DependentSingletons,// 名称（DependencySingletonFuncArray）
	FuncInfo,// 名称（FunctionLinkArray）
	nullptr,// 名称（PropertyArray）
	nullptr,// 名称（ImplementedInterfaceArray）
	UE_ARRAY_COUNT(DependentSingletons),// 名称（NumDependencySingletons）
	UE_ARRAY_COUNT(FuncInfo),// 名称（NumFunctions）
	0,// 名称（NumProperties）
	0,// 名称（NumImplementedInterfaces）
	0x001000A0u,// 名称（ClassFlags）
	METADATA_PARAMS(Z_Construct_UClass_UMyTest_Statics::Class_MetaDataParams, UE_ARRAY_COUNT(Z_Construct_UClass_UMyTest_Statics::Class_MetaDataParams))
};
*/
...
/*
还需要提醒的是，在FObjectInitializer析构的过程中，会执行到UObject::PostInitProperties的这个方法中，也就是我们要将属性赋值，最好写在PostInitProperties这个方法里面。
*/

```
### 总结
1. 咱们UMyTest编译生成两个类型，在cpp文件中创建了这两个TClassCompiledInDefer 和 FCompiledInDefer 类型的静态对象
2. 引擎运行的时候会加载CoreUObject这个模块，StartupModule方法里就会将模块里的UObject的UClass注册，并且会监听其他模块的加载完成，每有模块加载完成就会调用ProcessNewlyLoadedUObjects方法，来注册模块中的UClass
3. 注册UClass就分成了创建UClass通过GetPrivateStaticClassBody这个方法，进一步填充UClass中信息通过UObjectForceRegistration这个方法，然后就是填充UClass中的反射信息通过UObjectLoadAllCompiledInDefaultProperties这个方法，

2 在加载模块dll的时候，通过这两个静态对象的构造函数，将MyTest的UClass信息分别添加到静态全局数组（DeferredClassRegistration和DeferredCompiledInRegistration数组）中

3 引擎初始化完成后，会继续加载新的模块dll，每次加载完通过代理都会执行到切入函数（ProcessNewlyLoadedUObjects）

4 切入函数就是将每个模块dll文件中收集UClass信息，依次通过UClassRegisterAllCompiledInClasses调用UClass构造函数创建出UClass，然后通过UObjectProcessRegistrants完善UClass里面的各种信息并添加到全局GUObjectArray这个数组里面，进而通过UObjectLoadAllCompiledInDefaultProperties来创建CDO

5 创建CDO的过程最后面，就会调用UClass里面记录的UMyTest构造函数，完成我们CDO的创建

6 需要注意的是UObject的UClass的创建过程和上述的不同，方法用的都是一样的 ，但流程不同
