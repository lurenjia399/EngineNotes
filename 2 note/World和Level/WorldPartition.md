...
4.3.2 第二部分是CreateStreamingGrid方法，每一个Grid都会创建一个StreamingGrid，每个StreamingGrid中会记录RuntimeCell。**最终结果就是根据DataChunk创建出RuntimeCell，把RuntimeCell添加到StreamingGrid中**。
...
2.2 PostUpdateStreamingStateInternal_GameThread 这个方法主要就是执行UWorldPartitionStreamingPolicy::UpdateStreamingPerformance这个方法，主要目的就是性能判断，性能的预计算是在UpdateStreamingState方法中，计算出streamingsource距离cell的距离，以及距离和streamingsource半径的比例。然后在UpdateStreamingPerformance方法中判断，
...
6 我们在场景中有一个地板和三个立方体，我们在streaming加载的时候会出现立方体先加载出来，导致立方体掉落，掉地下的问题这个该怎么解决？
...
- 5 属于一个GridCell的Actor可能覆盖多个GridCell范围。IsStreamingCompleted是根据位置查询相交的GridCell，然后从GridCell中查找