...
- 关卡是如何切换的？
> 分成了客户端切换和服务器切换两部分，在服务器上ServerTravel切换是赋值World上的NextURL以及向客户端发送RPC切换关卡，在客户端上切换是赋值World上的TravelURL。真正的驱动切换流程是在World的UEngine::TickWorldTravel方法，这个方法分成了四部分，第一部分是无缝切换，第二部分就是读取NextURL，执行Browse方法最终执行loadmap方法，第三部分是读取TravelURL，执行Browse方法创建出PendingNetGame，创建PedingNetGame的时候也会创建NetDriver，NetDriver初始化会走到InitConnect方法来和服务器建立链接，等到握手成功建立链接后才会等待TickWorldTravel来处理PendingNetGame。等到客户端地图加载完成后就会销毁PendingNetGame，然后把NetDriver赋值给World中，并且会通知服务器客户端地图加载完成，服务器就会创建PlayerController并设置可同步属性之类的。第四部分是处理PendingNetGame。
- 流式关卡是怎么切换的？
