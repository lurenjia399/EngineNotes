...
void UNavigationSystemV1::Build()
{
	...
	*/
	...
	// 重新构建导航数据，主要对之前准备好的 NavData 调用他们的 NavigationData::RebuildAll ，用他们具体的 NavDataGenerator （如 RecastNavMeshGenerator ）去 RebuildAll 重建所有已知的导航数据。方法的最终会填充这个PendingDirtyTiles数组，把需要更新的Tile记录下来
	RebuildAll();
	// 阻塞等待所有构建完成
