...
4 DoWork方法的内容就是GenerateTile，GenerateTile主要分成两部分，第一部分就是把Tile中的几何体体素化生成Span，过滤可行走的Span，压缩Span记录相邻信息，然后将压缩Span通过分水岭算法压缩成不同的区域，再将区域合并压缩成Layer。第二部分就是将Layer解压缩，然后通过分水岭算法划分出区域，优化生成区域轮廓，然后
```
