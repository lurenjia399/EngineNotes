...
3 SetUp方法的内容就是，根据Tile的大小在NavOctTree中查到，查到的结果就是Tile所覆盖的Actor或者是Comp，然后遍历结果，将每个结果的Geometry都收集到RawGeometry数组中。也就是收集几何体数据为体素化做准备。
4 DoWork方法的内容就是GenerateTile，GenerateTile主要分成两部分，第一部分就是把Tile中的几何体体素化生成Span，过滤可行走的Span，压缩Span记录相邻信息，然后将压缩Span通过分水岭算法压缩成不同的区域，再将区域合并压缩成Layer。第二部分就是将Layer解压缩，然后通过分水岭算法划分出区域，优化生成区域轮廓，然后三角化生成PolyMesh，进一步生成DetailMesh，最后生成NavMeshData导航数据。
```
	...
	FNavMeshBuildContext& BuildContext, const dtLinkBuilderData& InLinkBuilderData)
{
	...
	*/
	...
	2 其中分水岭的距离场表示每个span到最近边界(不可走处 或 area 类型切换处)的距离。
	...
	const dtLinkBuilderData& InLinkBuilderData, int32 LayerIdx)
{
	...
	*/
	...
	2 这些Area也包括运行时动态修改的，比如破坏物被破坏之后
	*/
	MarkDynamicAreas(*GenerationContext.Layer);
	/*
	1 再次分水岭的操作，和上边RecastBuildLayers体素化之后分水岭的区别就是第一次分水岭将Span划分成不同的Layer，第二次是在Layer上分水岭会考虑ModiferVolume，将Layer转化成Region。
	2 这一次的分水岭操作，是在层的基础上，遍历格子,先算出每个格子到最近边界(不可走处 或 area 类型切换处)的距离;以此为海拔跑分水岭洪水填充,把格子划分成平面连通的 region。
	3 和第一次分水岭的区别。两者都是为了划分出区域，第一次划分出区域后再根据Span的垂直关系划分出Layer，而第二次就是在Layer的基础上划分区域为后边的操作使用。
	4 第一个已经划分出区域了为什么第二次还要划分。第一次是划分出了区域但是为了优化压缩的层数据，所以就没有存下来，第二次划分出区域是在ModifierVolume之后，区域有可能根据Volume动态改变，所以还需要根据层在划分一次。
	*/
	{
	...
	/*
	1 创建轮廓contour。分水岭划分出的区域是一个个的小正方形组成的，所以他们的轮廓是由锯齿的，这里就是为了减少锯齿，削减了正方形顶点数量，看其他平滑一点。
	*/
	{
