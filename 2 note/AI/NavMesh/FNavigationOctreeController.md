...
4 PendingUpdatas数组的处理是在UNavigationSystemV1::Tick进行的，具体是通过AddElementToNavOctree方法把Element添加到NavOctree中。其中如果有NavigationParent也会把Parent用相同的方法也添加到NavOctree中。在添加Octree后顺便也会把Actor的Bounds所覆盖的区域标记为脏
5 
```
