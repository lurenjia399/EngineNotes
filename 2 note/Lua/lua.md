...
3 如果LuaUObjectUserdata对象为空，则调用lua_newuserdata新生成一个LuaUObjectUserData对象（标记为Born），然后赋值对象中的stamp，uobj以及flag，并加入到FLuaObjectReferencer的ScriptCreatedObjects映射表中,也就是纳入到gc系统中。
