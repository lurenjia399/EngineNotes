...
{
	// 首先会尝试从Lua注册表中的全局弱表（g_udref）中获取到UObject的LightUserData对应的LuaUObjectUserdata
	...
	if (lua_isnil(L, -1) || !valid)
	{
		...
		// 将obj加入到FLuaObjectReferencer的ScriptCreatedObjects映射表中
...
1 首先会尝试从Lua注册表中的全局弱表（g_udref）中获取到UObject的LightUserData对应的LuaUObjectUserdata。如果找到了就直接返回了。
3 如果LuaUObjectUserdata对象为空，则调用lua_newuserdata新生成一个LuaUObjectUserData对象（标记为Born），然后赋值对象中的stamp，uobj以及flag。将userdata加入到FLuaObjectReferencer的ScriptCreatedObjects映射表中,也就是纳入到gc系统中。
4 通过UObject的UClass获取到ClassName，然后在注册表中找ClassName对应的元表。如果找不到就创建一个。
5 如果找到了，就将其设置为LuaUObjectUserdata的元表。然后在将LuaUObjectUserdata添加到注册表中的全局弱表里，key为UObject的lightuserdata。
## 通过userdata返回UObject
```cpp
/*
...
5 切换下一关是无缝切换，首先会收到NT_READY_NEXT_NODE这个消息，首先将玩家的移动tick关掉然后teleport到开始房间（同一个肉鸽里面，所有的开始房间和结束房间都是一样的，只是会有偏移和旋转不同），然后卸载旧结束房间->加载新的结束房间->加载新的主房间->卸载旧的主房间，然后才会收到SelfEnterScene正常进入场景的消息。
## 配置方面
...
![image.png](https://gitee.com/lurenjia399/image/raw/master/image/202503171516165.png)
link_config_id就是节点的连接结构，表示每个节点的下一个节点是哪个。
![image.png](https://gitee.com/lurenjia399/image/raw/master/image/202503171517331.png)
...
4 相应的每个scene_id就是对应每个房间的场景了，也就是ue中level是哪个，里面可以配开始房间，结束房间偏移之类的。
