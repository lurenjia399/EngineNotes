...
void FZoneGraphBuilder::Build(AZoneGraphData& ZoneGraphData)
{
	...
	// 创建当前ZoneStorage里面所有车道的Link车道，包括相邻车道（左侧车道，右侧车道，同向，反向），相连车道（同向流入流出，分流合流）
	// Connect Lanes
	...
	FZoneGraphStorage& ZoneStorage)
{
	...
	for (int32 LaneIndex = 0; LaneIndex < ZoneStorage.Lanes.Num(); LaneIndex++)
	{
		...
		/*
		1 通过当前遍历到的车道，先找到InternalLink数据，link数据就说明当前车道有相邻车道
		2 把当前遍历到的车道的所有相邻车道都添加到ZoneStorage里，每条车道最多相邻两个车道
		3 AdjacentLaneCount 增加计数，表示当前车道有几条相邻车道
		*/
		// Add internal links
		...
		2 起始车道就有两种情况，一种是同向相连流入，另一种是分流，同一个点流向不同方向
		3 后面省略了在车道终止点位置创建盒子查询的结果，这种也是有两种情况一种是同向相连是流出，另一种是合流，不同点流向终止点
		*/
		QueryResults.Reset();
		LinkGrid.Query(
			FBox::BuildAABB(
						...
						DestLane.Tags & BuildSettings.LaneConnectionMask))
				{
					...
					if (UE::ZoneGraph::Internal::InRange(
						...
						&& UE::ZoneGraph::Internal::InRange(
							...
							ConnectionToleranceSqr, AdjacentRadiusSqr))
					{
						// 当前车道起始点朝左方向 点乘 （当前车道起始点 朝向 目标车道起始点），如果是小于90度说明目标车道起始点在当前车道起始点左边
