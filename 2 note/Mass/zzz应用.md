...
2 UMassTrafficVehicleControlProcessor SimpleVehicle计算目标速度，PIDVehicle计算油门刹车转向值，SimpleVehicle就是那些ISM车，PIDVehicle就是Actor车其中LowActor是mass模拟物理，HightActor是走VehicleMovementComp的真实物理。如果是PIDVehicle在变道过程中，就会将前瞻点设为变道位置，计算出Steering设置到PIDFragment中，真正用到这个Steering的是HtightActor
...
6 UMassTrafficActorVehiclePhysicsProcessor HightActor会在这个里将
```