...
#  火车
```cpp
1 构建火车路线，以火车站Actor为基准构建，通过FindLaneOverlaps方法找到车站Actor覆盖的Lanes，通过FindNearestLane找到路线的起始点。沿着起始点开始遍历Linklanes，直到遍历完或者回到起点。顺便将火车路线，火车交叉路口等信息记录下来，保存到SaveActor身上。
2 
```