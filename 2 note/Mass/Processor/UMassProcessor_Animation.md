...
2 执行UMassCrowdUpdateISMVertexAnimationProcessor::UpdateISMVertexAnimation方法，将entity的动画数据（5个float类型的）添加到S 记录到ISMComp的SharedData中，等待RepresentationSubsystem中执行SetCustomDataById，把数据根据InstanceID记录到ISMComp里
3 
```
