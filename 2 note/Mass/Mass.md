...
/*
	...
	2 uint32 CreatedArchetypeDataVersion = 0;// 就是一个计数器，每次我们创建新的ArchetypeData的时候，这个计数器+1，并且这个值一旦设置就不会被更改。
	...
	9 int32 EntityListOffsetWithinChunk;//这个属性在初始化的时候为0。字面意思在Chunk的开头就会存放所有的EntityHanle，这个属性就是一个偏移，可以把所有的EntityHandle不放在开头放在这个偏移的后边。
