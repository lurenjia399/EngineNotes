...
3 processor的依赖关系是通过 ResolveDependencies 这个方法来解析依赖关系的，是在ProcessorPhase的tick的ka。可以通过ExecuteOrder显示设置Processor的依赖关系，然后会通过Processor读写Fragment来确定依赖关系，最后进行拓扑排序。
```
