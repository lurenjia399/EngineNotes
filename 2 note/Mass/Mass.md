...
2 在创建Task的方法里面，在局部变量的内存中用placement new的方式创建了ExecutionContext并右移出来（只是借用局部变量ProcessingContext的内存创建ExecutionContext并移动出来，局部变量ProcessingContext里面的数据什么都没变）。
3 在两个Task的方法里面，传给第一个Task的ExecutionContext是引用，这是因为这个Task
