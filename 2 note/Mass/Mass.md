...
```cpp
/*
1 FMassArchetypeCompositionDescriptor CompositionDescriptor;//原型组成描述符，由五个Fragments, Tags, ChunkFragments, SharedFragments, ConstSharedFragments组成。这个Descriptor是EntityTemplate传进来的，而EntityTemplate的赋值是在Trait创建的时候，在Trait的创建方法中赋值的这个Descriptor,这几个Fragments都是由Trait创建的时候。在
*/
