...
//**************以上9个都在 FMassArchetypeData::Initialize 方法中赋值***********
	10 TArray<FMassArchetypeChunk> Chunks;//这个就是Archetype里的Chunk数组，会在生成Entity的时候创建一个Chunk，往Chunk里添加Fragment，ChunkFragment等。
...
void UMassSpawnerSubsystem::DoSpawning(
	const FMassEntityTemplate& EntityTemplate, 
...
```
3 遍历通知数组中，对通知数组处理，就是获取观察数组，查找通知数组中的fragment是否在观察数组中，如果在就执行观察数组中的processor，这是create观察类型的过程。
			...
			const FMassExecuteFunction& ExecuteFunction)
{
	...
	{
		// 遍历所有的ArcheTypeData，从中找出符合Query的，填充到ValidArchetypes数组中
		CacheArchetypes();
		...
		else
		{
			...
			// 遍历ValidArchetypes数组，对每一个ArcheTypeData都执行ExecuteFunction方法
	...
	const FGraphEventArray& InPrerequisites)
{
	...
	// 在Events依赖数组都执行完成后，在线程池中找到空闲的一个高优先级线程，在高优先级线程中执行lambda表达式（是空的）
	FGraphEventRef CompletionEvent = 
...
4 ProcessorPhase是tickfunction，没有设置工作线程执行，所以是gamethread执行的，执行完成的判断是等待所有的Processor都执行完。
```
